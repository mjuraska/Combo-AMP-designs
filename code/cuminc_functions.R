...
EffCIR <- function(obj, refLvl, cmpLvl=NULL, confLevel=0.95, nullHypEff=NULL, test = "twoSided") {
  ...
  #test: whether a two-sided test is performed or a one sided-test is performed
  #    "twoSided": the test provided is the two-sided Wald test of
  #       H0: Efficacy=xxx vs. HA: Efficacy != xxx, 
  #    "oneSided": the test provided is the one-sided Wald test of
  #       H0: Efficacy <= xxx  vs. HA: Efficacy > xxx
  ...
  if(test == "twoSided"){
      ...
      # compute test statistics for two-sided tests of H0: Eff = nullHypEff
      # equivalent to H0: logCIR = log(1-nullHypEff)
      testStat <- (hypLogCIR - lastCIR$logCIR)/lastCIR$se.logCIR

      # compute the pvalues for the test-statistics
      pvalue <- 1 - pnorm(testStat)
